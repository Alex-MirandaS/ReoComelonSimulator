...
public interface BitacoraRegistroPresosCrud extends JpaRepository<BitacoraRegistroPresos, Integer> {
    @Query(value = "select * from bitacora_registro_presos where id_registro_presos = ? ", nativeQuery = true)
    List<BitacoraRegistroPresos> getBitacoraRegistroPresosByIdRegistroPresos(Integer id_registro_presos);
}
